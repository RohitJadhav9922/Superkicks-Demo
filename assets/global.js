...
let closeBTN = document.querySelector('#icon-hamburger');
let MobileMenu = document.querySelector('.menu-drawer-container');
...
class VariantSelects extends HTMLElement {
  ...
  updateRaffleId() {
    if (this.currentVariant && document.querySelector('[name="variantId"]')) {
      document.querySelector('[name="variantId"]').value = this.currentVariant.id;
    }
  }
  updateOptions() {
  ...
  renderProductInfo() {
    fetch(`${this.dataset.url}?variant=${this.currentVariant.id}&section_id=${this.dataset.originalSection ? this.dataset.originalSection : this.dataset.section}`)
      ...
      .then((responseText) => {
        ...
        if (document.querySelector(".recaptcha p"))
          document.querySelector(".recaptcha p").style.display = "none";
      });
  }
  ...
  toggleAddButton(disable = true, text, modifyClass = true) {
    ...
    //for fcfs product
    if(sessionStorage.getItem("Added") == "true" && document.querySelector("[data-fcfsproduct='true']")){
...
class InfinitePageLoader extends HTMLElement {
  ...
  connectedCallback() {
    setInterval(function () {
      if(document.querySelector(".mobile-facets__sort")){
        document.querySelector(".mobile-facets__sort").addEventListener("click",function(){
            this.classList.toggle("active");
            document.querySelector(".sorting-select_wrapper").classList.toggle("active");
        })
      }

      //for svg change in mobile sorting
      let sortingbtn = document.querySelectorAll(".sorting-label");
      if(sortingbtn){
        for( let i=0;i<sortingbtn.length;i++){
          // sortingbtn[0].classList.add("active");
          sortingbtn[i].addEventListener("click",function(){ 
  ...
  addObserver() {
    const observer = new window.IntersectionObserver(([entry]) => {
      ...
      if (entry.isIntersecting) {          
          ...
          // entry.target.style.border = '5px solid blue';
      ...
      fetch(currentUrl.toString(), {
        ...
        response.text().then(function (content) {
          ...
          if (insertPosition == 'append') {
            _this4.collectionInnerElement.innerHTML += tempElement.querySelector('#product-grid').innerHTML; //Appending new sets of product cards
...
/* Blog Banner */
let articleCard = document.querySelectorAll(".featured-article-card");
let blogBanner = document.querySelector(".blog_image");
let blogBannerMobile = document.querySelector(".blog_image-mobile");
let blogTitle = document.querySelector(".title_of--blog")
let blogTitleMobile = document.querySelector(".title_of--blog-mobile")
let blogUrl = document.querySelector(".article_url");
let blogUrlMobile = document.querySelector(".article_url-mobile");
if(blogBanner || blogBannerMobile){
blogBanner.src = articleCard[0].querySelector("img").src.split('&')[0];
blogBannerMobile.src = articleCard[0].querySelector("img").src.split('&')[0];
blogUrl.href = articleCard[0].getAttribute('data-url');
blogUrlMobile.href = articleCard[0].getAttribute('data-url');
articleCard.forEach((article)=>{
    articleCard[0].classList.add("active");
    article.addEventListener("click",function(){
        blogBanner.style.display = "none";
        blogBannerMobile.style.display = "none";
        let currentActive = document.querySelector(".active");
        currentActive.classList.remove("active");
        article.classList.add("active");
        blogBanner.src = article.querySelector("img").src.split('&')[0];
        blogBannerMobile.src = article.querySelector("img").src.split('&')[0];
        blogTitle.innerHTML = article.querySelector(".article-title").textContent;
        blogTitleMobile.innerHTML = article.querySelector(".article-title").textContent;
        blogUrl.href = article.getAttribute('data-url');
        blogUrlMobile.href = article.getAttribute('data-url');
        blogBanner.style.transform = "translateY(-100%)";
        blogBannerMobile.style.transform = "translateY(-100%)";
        setTimeout(()=>{
            blogBanner.style.display = "block";
            blogBannerMobile.style.display = "block"; 
            
        },200)
        setTimeout(()=>{
            blogBanner.style.transform = "translateY(0)";
            blogBannerMobile.style.transform = "translateY(0)";
        },400)
    });
})
}
